...
class BackendReceiptProcessor {
    ...
    private static var backendURL: String {
        return AppConfig.getBackendURL() ?? AppConfig.Backend.production
    }
    ...
    private static func sendToBackendAPI(_ extractedText: String, forUser userId: String, completion: @escaping (Result<Receipt, AppError>) -> Void) {
        ...
        guard let url = URL(string: "\(backendURL)/process-receipt") else {
            completion(.failure(.apiError("Invalid backend URL")))
            return
        }
        ...
        Auth.auth().currentUser?.getIDToken { token, error in
            ...
            print("✅ Got Firebase token (length: \(token.count))")
    ...
    private static func parseReceiptResponse(data: Data, completion: @escaping (Result<Receipt, AppError>) -> Void) {
        do {
            // Parse the backend response which includes the receipt
            if let json = try JSONSerialization.jsonObject(with: data) as? [String: Any],
               let receiptData = json["receipt"] as? [String: Any] {
                
                let receiptJson = try JSONSerialization.data(withJSONObject: receiptData)
                let receipt = try JSONDecoder().decode(Receipt.self, from: receiptJson)
                
                print("✅ Receipt parsed successfully: \(receipt.storeName ?? "Unknown store")")
                completion(.success(receipt))
                
            } else {
                // Try direct parsing if backend returns receipt directly
...
// 📱 Usage Example:
/*
// Replace your existing ReceiptProcessor calls with:
